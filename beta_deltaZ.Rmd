...
Now we simulate a known selection gradient which is highly correlated with cranial size, by perturbing the isometric vector with a small multivariate normal perturbation vector. This is reasonable, since we are dealing with overall size selection. We can control the size of the perturbation via the variance of the perturbation.
...
If we have an error-free estimate of the G-matrix, we can use the $\Delta z$ to reconstruct $\beta$ exactly:
...
The estimated $\beta$ is completely different from the actual $\beta$, and their correlation is about what we would expect for random vectors in 35 dimensions. If fact, the $\Delta z$ is a better predictor of the direction of the actual $\beta$ than the estimated selection gradient we obtain from solving the Lande equation.
...
We can also improve the estimation of the selection gradient by having extremely large sample sizes and, consequently, well estimated matrices:
...
We can simulate the direction of divergence $\delta z$ between two populations that were subject to opposite selection gradients (both related to overall size) and compare its alignment with the actual $\beta$. We use independent errors in both selection  gradients, with a larger variance than in the other examples, to account for the fact that selection is not completely symmetrical:

```{r, out.width = "330px"}
simulateBetaEstimation = function(beta_center, G, beta_var = 1, Gsample = 150) {
  pop = rmvnorm(Gsample, mean = rep(0, 35), sigma = G)
  G_estimated = cov(pop)
...
The direction of divergence $\delta z$ describes the same direction as $\Delta z$, and when the selection gradient is related to  size, it is also a better predictor of the actual direction of selection than the estimated $\beta$.
